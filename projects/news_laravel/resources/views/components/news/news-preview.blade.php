...
<div class="bg-gray-300 shadow-md rounded-lg m-1">

    @if ($categoryIsVisible)
        <a href="{{ route('categories.show', ['category' => $news->category]) }}"
           class="block bg-gray-200 uppercase p-2 rounded-lg rounded-b-none hover:bg-gray-300 font-semibold"
        >{{ $news->category->title }}</a>
    @endif



    <a class="block hover:bg-gray-300 p-4 rounded-lg
